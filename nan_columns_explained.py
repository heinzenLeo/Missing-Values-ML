...
def nan_columns(X_train, X_valid):
    ...
    def retrieve_name(var): # function to retrieve the arguments' names from nan_columns function -----> credits to: scohe001 from stackoverflow
    ...
    cols_with_missing = [] # a list that we'll add columns with nan values from X_train and X_valid
    ...
    for i in xs:
        ...
        # notice that missing type is pd.Series
        ...
        if missing.empty == True:
            ...
            if z == 0:
                ...
                print('|' + xs_names[z] + '|', '-->', '(columns w/ nan values, number of missing values):', '\n')
            z += 1
            # again, the piece of code above is just for fancier output
        ...
        # here you're comparing the 2nd input with the 1st input (v.difference(x) --> v=cols_with_misisng[1] // x=cols_with_missing[0])
        # it works the same as the above piece of code, but it is just inverted
        
        if len(difference_2) == 1: # the code below is just for fancier output, again
